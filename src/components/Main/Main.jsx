...
export default function Main() {
  const { buscaAtiva } = useContext(SearchContext);
  ...
  return (
    <main className="playlist-container">
      {buscaAtiva && artistas.length > 0 && (
        <section id="result-artist">
          <h2>Resultado da busca</h2>
          <div className="grid-container">
            {artistas.map((artista) => (
              <div className="artist-card" id={artista.id} key={artista.id}>
                <img src={artista.imagem} alt={artista.name} />
                <span className="fa fa-solid fa-play"></span>
                <span className="artist-name">{artista.nome}</span>
                <span className="artist-categorie">{artista.categoria}</span>
              </div>
            ))}
          </div>
        </section>
